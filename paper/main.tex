...
\textbf{(1)无数据泄漏。} 训练集(含验证)与测试集的对话\textbf{完全重复数为 0},排除了因样本泄漏导致的虚高;但训练集内部存在 25.3\% 的重复对话,反映该数据集系合成/半合成构造。
...
我们沿用三个代际的代表模型,在欺诈样本(训练 7{,}343 / 测试 1{,}391)上重新训练并评估,损失函数采用\textbf{类别加权交叉熵}以缓解不均衡。表~\ref{tab:ftype} 报告了准确率与宏平均/加权平均 F1。
