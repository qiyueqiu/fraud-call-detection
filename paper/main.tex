...
本文使用课程提供的中文通话互动数据集,原始对话共 16{,}189 通,清洗后按标签分层划分为训练集 12{,}273 通、验证集 1{,}364 通与测试集 2{,}552 通。每条样本包含对话正文(\texttt{\seqsplit{specific\_dialogue\_content}})、互动策略(\texttt{interaction\_strategy})、通话类型(\texttt{call\_type})、是否欺诈(\texttt{is\_fraud})与欺诈类型(\texttt{fraud\_type})五个字段。预处理包括:
...
\textbf{(3)两类分布差距显著。} 欺诈通话含上述关键词的比例为 58.3\%,而正常通话仅 2.2\%。关键词虽非充分线索,但其在两类间的极端不均衡,正是检测器易学、也易被对抗规避攻击针对的弱点。
