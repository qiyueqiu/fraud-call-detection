...
\multirow{2}{*}{预训练模型} & RoBERTa-wwm-ext & 0.743 & \textbf{0.748} & 0.754 \\
...
\textbf{现象分析}:与二分类的近完美形成鲜明对比,多分类的最优 Macro-F1 仅约 0.75,\textbf{任务难度显著提升},从而有效拉开了模型差距:具备深层语义建模能力的预训练模型(RoBERTa/MacBERT)与传统逻辑回归相当且略优,而依赖局部 n-gram 的字符级 TextCNN 明显落后(Macro-F1 仅 0.664)。这印证了"任务越难,语义表示的价值越凸显"——在二分类的简单线索下各模型无从区分,而在需要细粒度语义辨析的多分类任务上,模型能力的层次才得以显现。

更具启发性的是\textbf{逐类 F1 的悬殊分化}(图~\ref{fig:ftype_perclass})。\emph{投资诈骗}(F1$\approx$0.98)、\emph{彩票诈骗}(F1$\approx$0.9$\sim$1.0)、\emph{绑架诈骗}(F1$\approx$0.90)识别近乎完美,因为它们各自携带\textbf{专属强特征}("年化收益率/投资顾问""中奖/兑奖""人质/赎金");而\emph{钓鱼诈骗}最难(F1 仅 0.38$\sim$0.47),\emph{身份盗窃}次之(F1 0.51$\sim$0.56,且样本最少),二者语义边界模糊、与其他类型交叉。混淆矩阵(图~\ref{fig:ftype_cm})进一步显示:\textbf{客服诈骗与银行诈骗之间存在显著互相误判}——二者都以"账户异常、协助核验、退款返现"为话术骨架,语义高度同质,是拉低整体性能的主要来源。这说明欺诈类型的细分本质上是一个\textbf{语义密集型}任务,远非关键词匹配可解,为后续研究(如引入类型层次结构、对长尾类别做数据增强)留下了空间。
