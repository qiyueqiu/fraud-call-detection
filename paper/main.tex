...
本文使用课程提供的中文通话互动数据集,原始对话共 16{,}189 通,清洗后按标签分层划分为训练集 12{,}273 通、验证集 1{,}364 通与测试集 2{,}552 通。每条样本包含对话正文(\texttt{specific\_dialogue\_content})、互动策略(\texttt{interaction\_strategy})、通话类型(\texttt{call\_type})、是否欺诈(\texttt{is\_fraud})与欺诈类型(\texttt{fraud\_type})五个字段。预处理包括:
...
\textbf{现象分析}:如此一致且接近完美的性能,看似令人振奋,实则\textbf{值得审慎追问其根因}。为厘清这一点,我们对数据集做了捷径诊断(§4.4),结论是:高性能并非源于数据泄漏,而是欺诈与正常通话在多个层面系统性可分——这种可分性在原始分布上稳固,却未必能抵御针对性的对抗改写。基于综合性能,我们选取覆盖三个代际的代表性模型进入鲁棒性评估。
...
我们从测试集中抽取 400 条欺诈对话,使用 DeepSeek 大模型按 §3.2.4 的八种策略分别改写(每种 400 条,共 2{,}000 条,改写成功率 100\%),在\textbf{不重新训练}的前提下,用已训练好的五个模型对改写样本进行预测。
...
更值得警惕的是,部分翻转样本\textbf{原本关键词数即为 0},仅靠口语化改写就被击穿。例如一通投资诈骗,原文“我是天海投资咨询公司的投资顾问……年化收益率可以达到百分之二十”被改为“我是天海那边的,搞投资这块的……年化能到二十个点”,未删任何敏感词,却使 BiLSTM 的欺诈置信度从 0.9997 跌至 0.0407。这说明浅层模型不仅依赖关键词,还依赖\textbf{正式、书面化的话术句式}作为判别线索——一旦改写为随意的口语表达,即便语义未变,模型也会失守。这与 §4.4 的捷径诊断结论(模型部分依赖软性话术特征)相互印证。
...
§4.6 的 LLM 语义改写是\textbf{人工设计、语义级}的攻击。为从另一互补视角检验鲁棒性,本节实现经典的 \textbf{TextFooler}~\cite{jin2020textfooler}(AAAI 2020)——一种\textbf{自动化、词级、黑盒}的对抗攻击,它不依赖大模型,仅通过对分类器的查询反馈,贪心地以最少的同义词替换使预测翻转。这是 NLP 对抗鲁棒性领域的标准基线,可定量刻画“攻陷一个检测器需要多小的扰动、多少次查询”。
...
这一结果从\textbf{自动化对抗}的维度有力佐证了本文的核心论点:\textbf{依赖表层词汇统计的浅层模型在词级扰动下不堪一击,而具备深层语义理解的预训练模型能透过同义替换把握不变的欺诈意图,因而稳健得多}。TextFooler(自动词级)与 §4.6 的 LLM 改写(人工语义级)两类攻击\textbf{殊途同归},共同指向同一鲁棒性排序:\emph{预训练模型 $\gg$ 浅层模型},且 BiLSTM 始终是最脆弱的一环。两种攻击的交叉印证,使本文关于模型鲁棒性分层的结论更具说服力。
